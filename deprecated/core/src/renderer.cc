...
Renderer::Renderer(const int width, const int height)
  ...
  // inputs_.push_back(new InputUnit3("Camera Direction",
  // &camera_.direction()));
  input_index_ = 0;
}

// main loop of renderer
void Renderer::RunExample() {
  LoadCoordinateAxis();
  ...
  auto position_ptr = std::shared_ptr<math::vec3>(camera_.PositionPtr());
  FragmentShaderLightFull fragment_shader_light(
      camera_.PositionPtr(), std::shared_ptr<Material>(&object_material));
  fragment_shader_light.AddLight(&spot_light);
  FragmentShaderLightTexture fragment_shader_light_texture(
      *(camera_.PositionPtr()), object_material2);
  ...
  while (device_.Quit() == false) {  // renderer
                                     // main loop,
                                     // implement
                                     // rendering
                                     // pipeline
    ...
    // DrawObject(object, vertex_shader_light, fragment_shader_light,
    //            Rasterizer::TRIANGLE_FILL, DRAW_TRIANGLE);
...
// use appointed vertex shader
// and fragment shader for
// rendering
void Renderer::SetShader(VertexShader *vertex_shader,
                         FragmentShader *fragment_shader) {
...
// Draw mesh using rendering
// pipeline
void Renderer::DrawObject(Mesh &mesh, VertexShader &vertex_shader,
                          FragmentShader &fragment_shader,
...
// set polygonmode in triangle
// mode or wireframe mode
