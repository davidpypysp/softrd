...
bool Clipper::ClipLineNegativeW(LinePrimitive *line) { // false if need to discard this line
	int negative_num = 0;
	int index = 0;
	for (int i = 0; i < 2; i++) {
...
bool Clipper::ClipLine(LinePrimitive *line) { // Cohen - Sutherland algorithm
	vec4 p[] = { line->v[0].position, line->v[1].position };
	...
	for (int i = 0; i < 2; ++i) {
		vec4 clip_position;
		if (codes[i] != 0) {
			if (CalcPerspectiveLineIntersection(codes[i], p[0], p[1], &clip_position) == false) return false;
...
bool Clipper::OutsideViewFrustum(const vec4 &position) { // if position is out of frustum
	/*
...
bool Clipper::BehindEyePoint(const vec4 &position) {
	return position.w <= 0;
}

int Clipper::RegionCode(const vec4 &position) { // Cohen-Sutherland algorithm
	int code = 0;
	if (position.x < -position.w) code |= REGION_BL;
	else if (position.x > position.w) code |= REGION_BR;

	if (position.y < -position.w) code |= REGION_BB;
	else if (position.y > position.w) code |= REGION_BT;

	if (position.z < -position.w) code |= REGION_BN;
	else if (position.z > position.w) code |= REGION_BF;

	return code;
}

bool Clipper::CalcPerspectiveLineIntersection(const int code, const vec4 &p1, const vec4 &p2, vec4 *result) {
	float a = 0.0;
	vec4 p;
	if (code & REGION_BL) {
